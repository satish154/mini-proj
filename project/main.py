...
local_server=True
app=Flask(__name__)
app.secret_key="satishthota5814"
...
#app.config.update(
 #    MAIL_SERVER='smtp.gmail.com',
  #   MAIL_PORT='465',
    # MAIL_USE_SSL=True,
     #MAIL_USERNAME='satishthota5814@gmail.com',
     #MAIL_PASSWORD='Satish@553'
 #)
#mail = Mail(app)
...
def signup():
        if request.method == 'POST':
            ...
            dob_hashed = generate_password_hash(dob)
            user = User(srfid=srfid, email=email, dob=dob_hashed)  # Include the email field
            db.session.add(user)
            db.session.commit()
